...
def to_put_with_constraints(Tgoal):

# Usage: compute the trajectory of gripper to put down the box with constraints
# Params:
#   Tgoal: Transformation matrix of the gripper at destination

    constraintfreedoms = np.zeros(6)
    constraintfreedoms[0] = 1 # Gripper does not rotate around x axis
    constraintfreedoms[1] = 1 # Gripper does not rotate around y axis
    constraintmatrix = np.linalg.inv(Tgoal)
    Tee = manipulator.GetEndEffectorTransform()
    constrainttaskmatrix = np.dot(np.linalg.inv(Tee), Tgoal)
    traj = manipprob.MoveToHandPosition(matrices=[Tgoal], 
                                    execute=False,
...
for i in range(-1, -41, -1):
    ...
    try:
        try:
            ...
            put_traj = to_put_with_constraints(Tgoal)
